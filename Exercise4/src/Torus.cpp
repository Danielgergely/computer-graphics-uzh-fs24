...
namespace cgCourse
{
	...
	Torus::Torus() : Shape()
	{
        ...
        int numSegmentsXY = circleXY.getVertices().size(); // number of segments in XY plane
        int numSegmentsXZ = circleXZ.getSegments(); // number of segments in XZ plane
        ...
        for(int i = 0; i < positions.size(); i++) {
            int k = i / numSegmentsXZ;

            glm::vec3 T = glm::normalize(glm::vec3(circleXZ.getVertices()[(k+1)%numSegmentsXZ].x - circleXZ.getVertices()[k].x, 0.0,
                                                   circleXZ.getVertices()[(k+1)%numSegmentsXZ].y - circleXZ.getVertices()[k].y));
